...
public:
    bool judgeCircle(string moves) {
        int x=0;//记录机器人横坐标，初始是0
        int y=0;//记录机器人纵坐标，初始是1
        for(char &c:moves){
            if(c=='U')y++;
            else if(c=='D')y--;
            else if(c=='L')x--;
...
换一种看法就是，把当前数组的元素从首到尾拼接起来，组成当前数的最小上界
1.我们需要下一个数比当前数大，那么就需要将后面的大数与前面的小数交换。
2.我们只需要下一个数增加的幅度尽可能的小，才能满足“下一个排列与当前排列紧邻”
  2.1在尽可能靠右的低位进行交换，因此需要从后向前遍历
  2.2将一个尽可能小的[大数]与前面的[小数]交换，比如123465，应当让4和5交换，而不是4和6交换
  2.3将大数换到前面后，需要将大数后面的所有数重置为升序，因为升序排列就是最小的排列

算法过程：
1.从后向前查找第一个[相邻升序]的元素对（i,j），满足A[i]<A[j]，此时[j,end)必然是降序
eg.124321，(1,2)是第一对满足的，必然要把2换掉，因为2后面是降序，已经是最大的了，说明在2w这个领域里，没有比它更大的了
2.在[j,end]从后向前查找第一个满足A[i]<A[k]的k。A[k]就是尽可能小的[大数]
3.交换A[i]和A[k]
4.由于[j,end)是降序，使其反转，变为升序，此时就是最小的排列了
5.如果步骤1无法找到对应元素对，说明整体都是降序，已经是最大的了，直接翻转整体变为升序，即最小排列

代码：
class Solution {
public:
    void nextPermutation(vector<int>& nums) {
        int postIndex=0;//用于记录第一个元素对中j的位置
        for(int i=nums.size()-1;i>=1;i--){
            if(nums[i-1]<nums[i]){//1.找到第一个元素对
                postIndex=i;
                break;
            }
        }

        if(postIndex==0){//5.如果第一个元素对未找到
            reverse(nums.begin(),nums.end());
        }else{//找到了
            //2.找尽可能小的大数
            int k=0;
            for(int j=nums.size()-1;j>=postIndex;j--){
                if(nums[j]>nums[postIndex-1]){
                    k=j;
                    break;
                }
            }
            //3.交换大小数
            swap(nums[postIndex-1],nums[k]);
            //4.反转[postIndex,end)
            reverse(nums.begin()+postIndex,nums.end());
        }
    }
};
```
